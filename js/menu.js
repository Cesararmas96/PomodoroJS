document.addEventListener("DOMContentLoaded", () => {
    ...
    let settingsDiv = document.getElementById("setting");

    let menuTimeout = 800;

    function manageOpacity() {
        let container = document.querySelector("#pomodoro-container");
        ...
        if (container.style.opacity === "1" || container.style.opacity === "") {
            container.style.animation = decreaseOpacity;
            header.style.animation = decreaseOpacity;
            container.style.opacity = lowOpacity;
            header.style.opacity = lowOpacity;
        } else {
            container.style.animation = increaseOpacity;
    ...
    closeSettings.addEventListener("click", () => {
        settingsDiv.style.animation = `slideMenuOff ${menuTimeout /
            1000}s ease`;
        manageOpacity();
        setTimeout(() => {
    ...
    //     "div.settings-container > div input"
    // );
